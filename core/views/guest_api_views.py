...
# GÜNCELLEME: Gerekli importlar eklendi
...
class GuestOrderCreateView(generics.GenericAPIView):
    ...
    def post(self, request, table_uuid):
        ...
        if final_order_to_return:
            ...
            # Sipariş başarıyla kaydedildikten sonra bildirim gönder.
            transaction.on_commit(
                lambda: send_order_update_notification(
                    order=final_order_to_return,  # order_id yerine order nesnesi
                    created=(response_status_code == status.HTTP_201_CREATED)
                )
            )
            # --- GÜNCELLEME SONU ---
...
class GuestMenuView(generics.ListAPIView):
    # Bu view'da değişiklik yok, aynı kalabilir.
...
class GuestTakeawayOrderUpdateView(generics.GenericAPIView):
    ...
    def post(self, request, order_uuid):
        try:
            ...
            # --- GÜNCELLEME BAŞLANGICI ---
            # Sipariş güncellendikten sonra bildirim gönder.
            transaction.on_commit(
                lambda: send_order_update_notification(
                    order=order_to_update,  # order_id yerine order nesnesi
                    created=False,  # Bu mevcut bir siparişi güncelleme işlemidir.
                    update_fields=['items_added', 'status']
                )
            )
            # --- GÜNCELLEME SONU ---
